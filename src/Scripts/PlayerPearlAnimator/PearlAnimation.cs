...
public abstract class PearlAnimation
{
    ...
    public virtual void Update(Player player)
    {
        ...
        for (int i = 0; i < playerModule.Inventory.Count; i++)
        {
            ...
            if (!abstractObject.TryGetPearlGraphicsModule(out _))
                new PearlGraphics(abstractObject);
    ...
    public virtual void UpdateSymbolEffects(Player player)
    {
        ...
        for (int i = 0; i < playerModule.Inventory.Count; i++)
        {
            ...
            addon.SymbolColor = effect.MajorEffect == PearlEffect.MajorEffectType.CAMOFLAGUE ? Color.white : Hooks.GetObjectColor(abstractObject);
            ...
            var returnPos = Hooks.TargetPositions.TryGetValue(abstractObject, out var pos) ? pos.Value : player.GetActivePearlPos();
