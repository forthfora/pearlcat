...
public static class Hooks_Menu
{
    public static void ApplyHooks_Menu()
    {
        ...
        try
        {
            _ = new Hook(
                ...
                typeof(Hooks_Menu).GetMethod(nameof(GetSlugcatPageHasMark), BindingFlags.Static | BindingFlags.Public)
            );
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"{nameof(Hooks_Menu)};{nameof(GetSlugcatPageHasMark)}: {e}\n{e.StackTrace}");
        }
        ...
        try
        {
            _ = new Hook(
                ...
                typeof(Hooks_Menu).GetMethod(nameof(GetSlugcatPageHasGlow), BindingFlags.Static | BindingFlags.Public)
            );
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"{nameof(Hooks_Menu)};{nameof(GetSlugcatPageHasGlow)}: {e}\n{e.StackTrace}");
        }
        ...
        {
            Plugin.Logger.LogError($"{nameof(Hooks_Menu)};{nameof(SlugcatSelectMenu_StartGame)}: {e}\n{e.StackTrace}");
        }
    }
    ...
    private static void MenuIllustration_Update(On.Menu.MenuIllustration.orig_Update orig, MenuIllustration self)
    {
        ...
        if (fileName == "Intro6")
        {
            self.visible = !Utils.IsModEnabled_MiraInstallation;
        }
    ...
    private static void SlugcatSelectMenu_Update(On.Menu.SlugcatSelectMenu.orig_Update orig, SlugcatSelectMenu self)
    {
        ...
        var miraSkipAvailable = !disableSave && Utils.IsModEnabled_MiraInstallation && isPearlcatPage && !self.restartChecked && !miscProg.HasTrueEnding && !miscProg.UnlockedMira && ModManager.MSC;
        ...
        if (page is SlugcatSelectMenu.SlugcatPageContinue continuePage && module.OriginalRegionLabelText != null)
        {
            ...
            if (Utils.ShowMiraVersionWarning)
            {
