...
public static partial class Hooks_Player
{
    ...
    public static void UpdateRevive(Player self, PlayerModule playerModule, PearlEffect effect)
    {
        ...
        if (playerModule.ActiveObject == null || !playerModule.ActiveObject.TryGetPlayerPearlModule(out var poModule)) return;
    ...
    public static void RageTargetLogic(DataPearl pearl, Player player, bool isSentry)
    {
            ...
            foreach (var physObj in roomObject)
            {
                ...
                else if (physObj.abstractPhysicalObject.GetPOEffect().MajorEffect == MajorEffectType.RAGE)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Active red check
                            if (!physObj.abstractPhysicalObject.TryGetPearlGraphicsModule(out var graphics) || !graphics.IsActiveRagePearl) continue;
        ...
        foreach (var roomObj in pearl.room.physicalObjects)
        {
            foreach (var physObj in roomObj)
            {
                ...
                {
                    if (!redDist.Key.abstractPhysicalObject.TryGetPlayerPearlModule(out var otherSentryModule)) continue;
